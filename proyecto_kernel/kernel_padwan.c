...
static int __init super_module_init(void) {
    ...
    if (major < 0) {
        printk(KERN_ALERT "Kernel Padwan load failed, can't register chardev\n");
        return major;
    }
...
static int dev_open(struct inode *inodep, struct file *filep) {
   printk(KERN_INFO "Proyecto Kernel device opened\n");
   return 0;
}
...
static ssize_t dev_write(struct file *filep, const char *buffer,
    ...
    printk(KERN_INFO "Sorry, Proyecto Kernel is read only\n");
    return -EPERM;
}

static int dev_release(struct inode *inodep, struct file *filep) {
   printk(KERN_INFO "Proyecto Kernel device closed\n");
   return 0;
}

static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset) {
    int errors = 0;
    char *message = "No Luke, yo soy tu padre ";
