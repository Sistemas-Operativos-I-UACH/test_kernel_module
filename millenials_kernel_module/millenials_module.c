...
static int __init millenials_init(void) {
    ...
    if (major < 0) {
        printk(KERN_ALERT "Millenials load failed\n");
        return major;
    }
    printk(KERN_INFO "Millenials has been loaded\n");
...
static int dev_open(struct inode *inodep, struct file *filep) {
    printk(KERN_INFO "Millenials device opened\n");
    return 0;
}

static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset) {
    printk(KERN_INFO "Millenials is read only\n");
    return -EFAULT;
}

static int dev_release(struct inode *inodep, struct file *filep) {
    printk(KERN_INFO "Millenials device closed\n");
    return 0;
}
